...
当电流从 `-2A` 切向 `0A` 时，会出现**冲到 -5A** 的过冲。
![1](./1.png)
![2A-(-2A)-2A-(-2A)-0波形](./2A-(-2A)-2A-(-2A)-0.png)
- A1是pwm，A2是Cur[1]通道，A3是ref_mA，A4是Cur[0]通道，A5是pi积分
 - 图2是为了说明在连续换向的时候并没有问题
...
- 切 0 瞬间（由判断语句'ref_mA >= 0'，15s的0时刻一瞬间看到对Cur[1]有值，发现有误差并且一直无法消除直接把误差拉满了）：
  - `Target - Real = 0 - Cur[1] < 0`

位置式 PID 会将输出瞬间拉向负方向，导致电流冲到 `-5A`。  
并且因为Cur[0]的电流越大，那么灌到Cur[1]的电流也会越大

---
![正负100mA 波形](./正负100mA.png)
---
从该图片可以看到在低速下受影响（因为低速下过冲电流忽略不计）
