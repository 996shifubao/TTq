...
#include "main.h"
#include "adc.h"
int32_t Encoder[6];   // 编码器数值
uint8_t EncoderDir[6]={1,0,0,0,0,0}; // 编码器方向  0->正向  1->反向
volatile uint8_t g_log_flag = 0;
/* USER CODE END 0 */
...
void MX_TIM1_Init(void)
{
  ...
  GPIO_InitStruct.Pin = LL_GPIO_PIN_13|LL_GPIO_PIN_8|LL_GPIO_PIN_9|LL_GPIO_PIN_11
...
void MX_TIM2_Init(void)
{
  ...
  */
  ...
  /* USER CODE BEGIN TIM2_Init 1 */
	LL_TIM_EnableCounter(TIM2);//Enable EncoderA
...
void MX_TIM8_Init(void)
{
  ...
  LL_TIM_CC_EnableChannel(TIM8,LL_TIM_CHANNEL_CH1);
	...
	LL_TIM_EnableCounter(TIM1);
  LL_TIM_EnableIT_UPDATE(TIM1);//使能计时器更新中断
  ...
  GPIO_InitStruct.Pin = LL_GPIO_PIN_15|LL_GPIO_PIN_13|LL_GPIO_PIN_14;
  GPIO_InitStruct.Mode = LL_GPIO_MODE_ALTERNATE;
  GPIO_InitStruct.Speed = LL_GPIO_SPEED_FREQ_LOW;
  GPIO_InitStruct.OutputType = LL_GPIO_OUTPUT_PUSHPULL;
  GPIO_InitStruct.Pull = LL_GPIO_PULL_NO;
...
//	static uint16_t usbtic=0;
	if(LL_TIM_IsActiveFlag_UPDATE(TIM1))
	{
		...
		if(InitOK==1)
		{
			if(++tic>79)  // 18-> 4K  2->26.67K   频率=80K/(num+1)
			{
			tic=0;
				if(TargetBusy==0)
				{
					Get_Encoder_Dat(Encoder,EncoderDir);
          ControlSimple_Step(Encoder[0]);
//          Test_Step1kHz();
...
//			if(++usbtic>399) // 19->4K 9->8K
//      {
////        usbtic=0;
////        USB_RX_Task(); // 没打开上位机usb 7.5us   打开上位机12us
////        USB_TX_Task();
////        Set_A10
//      }
			}
...
{
    buf[0] = (int32_t)LL_TIM_GetCounter(TIM2);
    if (dir[0] == 0) buf[0] = -buf[0];
